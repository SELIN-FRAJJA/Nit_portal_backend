...
const StudentFinalizationWindowSchema = new mongoose.Schema({
  startDate: { type: Date, required: true },
  endDate: { type: Date, required: true }
});
const StudentFinalizationWindow = mongoose.model('StudentFinalizationWindow', StudentFinalizationWindowSchema);
...
// Faculty Internship Duration window (separate from overall duration)
const FacultyWindowSchema = new mongoose.Schema({
  startDate: { type: Date, required: true },
  endDate: { type: Date, required: true },
}, { timestamps: true });

const FacultyWindow = mongoose.model('FacultyWindow', FacultyWindowSchema);

// GET faculty window
app.get('/api/admin/faculty-window', async (req, res) => {
  try {
    const window = await FacultyWindow.findOne({});
    if (!window) return res.json({});
    res.json(window);
  } catch (err) {
    res.status(500).json({ error: 'Failed to fetch faculty window' });
  }
});

// POST faculty window
app.post('/api/admin/faculty-window', async (req, res) => {
  const { startDate, endDate } = req.body;
  if (new Date(startDate) > new Date(endDate)) {
    return res.status(400).json({ error: 'Start date must not be after end date.' });
  }
  try {
    const window = await FacultyWindow.findOneAndUpdate(
      {}, { startDate, endDate }, { upsert: true, new: true }
    );
    res.json(window);
  } catch (err) {
    res.status(500).json({ error: 'Failed to update faculty window' });
  }
});
...
app.post('/api/applications/basic-submit', upload.fields([
  ...
  try {
    ...
    const aadhaarUrl = files['aadhaarFile'] ? `/uploads/aadhar/${files['aadhaarFile'][0].filename}` : '';
    const idCardUrl = files['idCardFile'] ? `/uploads/idcard/${files['idCardFile'][0].filename}` : '';
    const resumeUrl = files['resumeFile'] ? `/uploads/resume/${files['resumeFile'][0].filename}` : '';
    const nocUrl = files['nocFile'] ? `/uploads/noc/${files['nocFile'][0].filename}` : '';
    const profilePhotoUrl = files['profilePhotoFile'] ? `/uploads/photos/${files['profilePhotoFile'][0].filename}` : '';
    ...
    const newApp = new Application({
      ...
      aadhaarUrl, idCardUrl, resumeUrl, nocUrl, profilePhotoUrl,
    });
    ...
    if (student) {
      ...
      if (profilePhotoUrl) student.profilePhotoUrl = profilePhotoUrl;
      await student.save();
    }
...
app.get('/api/applications/accepted-excel', async (req, res) => {
  try {
    ...
    worksheet.columns = [
      ...
      { header: 'Alt Phone', key: 'otherPhone', width: 15 },
      { header: 'Aadhaar Number', key: 'aadharNumber', width: 20 },
      { header: 'Area of Interest', key: 'areaOfInterest', width: 25 },
      { header: 'Experience', key: 'experience', width: 30 },
      { header: 'Father Name', key: 'fatherName', width: 20 },
      { header: 'Mother Name', key: 'motherName', width: 20 },
      { header: 'Parent Phone', key: 'fatherPhone', width: 15 },
      { header: 'Address', key: 'address', width: 40 },
      { header: 'Accommodation', key: 'studentType', width: 20 },
    ...
    applications.forEach(app => {
      worksheet.addRow({
        ...
        department: app.department || app.studentDepartment,
        currentYear: app.currentYear,
        cgpa: app.cgpa,
        phone: app.phone,
        otherPhone: app.otherPhone,
        ...
        domain: app.domain,
        startDate: app.startDate ? new Date(app.startDate).toLocaleDateString() : '',
        status: app.status
      });
    });
